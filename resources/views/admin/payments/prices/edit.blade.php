...
<div class="title-block">
    <h1 class="title">@lang('payments.prices.edit')</h1>
</div>
        ...
        <div class="col-md-6">
            <div class="card card-block">
                <div class="title-block">
                 ...
                 <form role="form" method="post" action="{{ route('admin.payments.prices.update', ['id' => $paymentPrice->id]) }}">
                    {{ csrf_field() }}
                    <div class="input-group @if ($errors->has('price')) is-invalid @endif">
                        <label for="price">@lang('payments.prices.price'):</label>
                        <input type="text" class="form-control" name="price" value="{{ $paymentPrice->price }}" id="price">
                    ...
                    @endif
                    <div class="form-group @if ($errors->has('payment_type')) is-invalid @endif">
                        <label for="payment_type">@lang('payments.types.type'):</label>
                        <select class="form-control" id="payment_type" name="payment_type">
                            ...
                            <option value="{{$row->id}}" @if($row->id == $paymentPrice->type->id) selected="true" @endif>{{$row->name}}</option>
                            @endforeach
                            @endif
                        </select>
                    ...
                    <div class="form-group @if ($errors->has('payment_period')) is-invalid @endif">
                        ...
                        <select class="form-control" id="payment_period" name="payment_period">
                            ...
                            <option value="{{$row->id}}" @if($row->id == $paymentPrice->period->id) selected="true" @endif>{{$row->name}}</option>
                            @endforeach
                            @endif
                        </select>
