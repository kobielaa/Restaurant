...
<div class="title-block">
    <h1 class="title">@lang('subdomains.edit')</h1>
</div>
        ...
        <div class="col-md-6">
            <div class="card card-block">
                <div class="title-block">
                 ...
                 <form role="form" method="post" action="{{ route('subdomains.update', ['id' => $subdomain->id]) }}">
                    {{ csrf_field() }}
                    <div class="form-group @if ($errors->has('user')) is-invalid @endif">
                        ...
                        <select class="form-control" id="user" name="user" required="required">
                            ...
                            <option value="{{$row->id}}" @if($subdomain->user_id == $row->id) selected="true" @endif>{{$row->email}} | {{$row->first_name}} {{$row->last_name}}</option>
                            @endforeach
                            @endif
                        </select>
                        @if ($errors->has('user'))
                    ...
                    <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                        ...
                        <input type="text" value="{{$subdomain->name}}" id="name" name="name" class="form-control underlined">
                        @if ($errors->has('name'))
