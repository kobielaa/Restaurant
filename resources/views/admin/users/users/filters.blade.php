<div class="filters"> 
    <div class="form-group">
        ...
        <div class="input-group ">    
            <label for="type">@lang('users.types.type')</label>    
            <select class="form-control" id="type" name="type">
                <option value="-1" disabled="true" selected="true">@lang('general.form.select')</option>
                @if(count($globalData['user_types']))
                    @foreach($globalData['user_types'] as $row)
                        <option value="{{$row->id}}" 
        ...
        <div class="input-group ">  
            ...
            <select class="form-control" id="job" name="job">
                <option value="-1" disabled="true" selected="true">@lang('general.form.select')</option>
                @if(count($globalData['jobs']))
                @foreach($globalData['jobs'] as $row)
                    <option value="{{$row->id}}" 
                        @if(strlen(app('request')->input('job')) == $row->id)selected="true"@endif>
                            {{$row->name}}
                        </option>
                @endforeach
                @endif
